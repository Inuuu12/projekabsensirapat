...
<footer class="bg-ijo-tua dark:bg-[#0f1c19] text-white mt-auto border-t border-white/10 dark:border-[#233a34] transition-colors duration-200">
    <div class="w-full max-w-[1680px] mx-auto px-4 sm:px-6 lg:px-8 2xl:px-10 pt-12 pb-8 space-y-10">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-8 lg:gap-12">
            ...
            <div class="lg:col-span-4 space-y-4">
                ...
                <p class="text-xs text-gray-300 leading-relaxed pr-2">
                    Platform terpadu pengelolaan agenda rapat, kehadiran pegawai, serta pelayanan kunjungan tamu rapat Pemerintah Kabupaten Bogor.
                </p>
        ...
        <div class="border-t border-white/10 pt-6 flex flex-col items-center justify-center text-center gap-2.5 text-xs text-gray-300">
            <p>&copy; {{ date('Y') }} {{ $appName }} - {{ $regionName }}. Hak cipta dilindungi.</p>
            
            <div class="pt-1">
                @if (Auth::guard('admin')->check())
                    <a href="{{ route('admin.dashboard') }}" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold transition-all shadow-xs">
                        <svg class="w-3.5 h-3.5 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5.121 17.804A13.937 13.937 0 0112 16c2.5 0 4.847.655 6.879 1.804M15 10a3 3 0 11-6 0 3 3 0 016 0zm6 2a9 9 0 11-18 0 9 11 0 0118 0z"/></svg>
                        <span>Dashboard Admin</span>
                    </a>
                @else
                    <a href="{{ route('admin.login') }}" class="inline-flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-white/10 hover:bg-oren-utama text-white hover:text-white border border-white/15 text-xs font-bold transition-all shadow-xs group">
                        <svg class="w-3.5 h-3.5 text-oren-utama group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
