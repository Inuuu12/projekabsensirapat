        ...
        @if ($agendaAktif)
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                ...
                <aside class="lg:col-span-5 space-y-5">
                    ...
                    <div id="presensi-section" class="bg-white rounded-3xl p-6 border border-gray-100 shadow-sm space-y-5">
                        ...
                        @if (session('success'))
                            <div class="rounded-2xl bg-ijo-sangatmuda border border-ijo-muda/40 text-ijo-tua px-4 py-3 text-xs font-bold flex items-center space-x-2">
                                <svg class="w-4 h-4 text-ijo-tua shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"></path></svg>
                                <span>{{ session('success') }}</span>
                            </div>
                        @endif

                        @if ($errors->any())
                            <div class="rounded-2xl bg-red-50 border border-red-200 text-red-700 px-4 py-3 text-xs space-y-1">
                        ...
                        @else
                            ...
                            <div id="panel-presensi-pegawai" class="space-y-4">
                                <div class="bg-gray-50 rounded-2xl p-4 border border-gray-100 text-center">
                                    <p class="text-[11px] font-semibold text-gray-700">QR Absen Pegawai</p>
                                ...
                                @if ($agendaAktif->status_qr === 'aktif' && $qrImageUrl)
                                    <div class="rounded-2xl border border-gray-100 bg-gray-50 p-4 text-center">
                                        <div class="inline-block rounded-2xl bg-white p-3 shadow-sm border border-gray-100">
                                            <img src="{{ $qrImageUrl }}" alt="QR Presensi {{ $agendaAktif->nama_agenda }}" class="mx-auto h-52 w-52 rounded-xl object-contain">
                                        </div>
                                        <div class="mt-4">
                                            <a href="{{ $qrPayload ?: route('publik.presensi.pegawai', ['agenda_id' => $agendaAktif->id_agenda]) }}" class="inline-flex w-full items-center justify-center rounded-xl bg-ijo-tua hover:bg-ijo-semitua px-4 py-2.5 text-xs font-bold text-white shadow-sm transition">
